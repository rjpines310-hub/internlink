...
if (!$date && !$status) {
    ...
    echo json_encode(['error' => 'Either date or status parameter is required']);
    exit();
}
...
try {
    ...
    if ($date) {
        $sql .= " AND ? BETWEEN DATE(assigned_at) AND DATE(due_date)";
        $params[] = $date;
        $types .= "s";
    }
    ...
    if ($status) {
        if ($status !== 'all') {
            ...
            $sql .= " AND status IN ('assigned', 'submitted', 'completed', 'missed')";
        }
    }
