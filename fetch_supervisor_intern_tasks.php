...
try {
    // Optional: Verify the intern is actually under this supervisor's supervision
    $verify_stmt = $conn->prepare("SELECT COUNT(*) FROM student WHERE student_id = ? AND supervisor_id = ?");
    $verify_stmt->bind_param("ii", $student_id, $_SESSION['user_id']);
    $verify_stmt->execute();
    ...
    if ($count == 0) {
        http_response_code(403);
        echo json_encode(['error' => 'You are not authorized to view tasks for this intern.']);
        exit();
    }
    ...
    if ($status_filter !== 'all') {
        $stmt->bind_param("is", $student_id, $status_filter);
    } else {
